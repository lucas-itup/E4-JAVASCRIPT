...
const buscarPizza = () => {
    ...
    validaciones(valueid, inputVacio)

    var found = pizzas.find(function(post, index) {
        if (post.id != valueid) return validaciones(valueid, inputVacio, post.id);

        precioPizza.textContent = post.precio;
    });
}

const validaciones = (valueid, inputVacio, post = null) => {

    if (valueid == "") return inputVacio.textContent = "Debes ingresar un valor numerico"
    console.log(post)
    if (post != null) return inputVacio.textContent = "El id ingresado no existe"
