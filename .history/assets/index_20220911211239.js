...
const buscarPizza = () => {
    ...
    if (validarInputBlanco(valueid, inputVacio)) {
        var found = pizzas.find(function(post, index) {
            console.log(post.id);
            console.log(valueid)
            post.id == valueid ? precioPizza.textContent = post.precio : inputVacio.textContent = "El id no existe";
        });
    }
}
...
const validarInputBlanco = (valueid, inputVacio, post) => {
    ...
    if (valueid == "") inputVacio.textContent = "Debes ingresar un valor numerico"
    return false;
}