...
const buscarPizza = () => {
    ...
    console.log(found)
    console.log(validarInputBlanco(valueid, inputVacio))
    if (found == undefined) {
        validarInputBlanco(valueid, inputVacio, found)
    }
}

const validarInputBlanco = (valueid, inputVacio, found) => {
    if (valueid == "") return inputVacio.textContent = "Debes ingresar un valor numerico"

    if (found == undefined) return inputVacio.textContent = "id"
}